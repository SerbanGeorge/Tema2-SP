...
t_original = 0:0.02:D; %esantionarea semnalului original
x_original = sawtooth((pi/12)*t_original,0.5)/2+0.5; %semnalul triunghiular original
t = 0:0.02:P; %esantionarea semnalului modificat
x = zeros(1,length(t)); %initializare a semnalului modificat cu valori nule
x(t<=D) = x_original; %modificam valorile nule cu valori din semnalul original
...
for k = -N:N %k este variabila dupa care se realizeaza suma
    x_fourier = x_original; %x_t e semnalul realizat dupa formula SF data
    x_fourier = x_fourier .* exp(-1i*k*w0*t_original); %inmultire intre doua matrice element cu element
    X(k+N+1) = 0; %initializare cu valoare nula
    for i = 1:length(t_original)-1
        X(k+N+1) = X(k+N+1) + (t_original(i+1)-t_original(i)) * (x_fourier(i)+x_fourier(i+1))/2; %reconstructia folosind coeficientii
    end
end
...
%Conform cursului de SS, teoria seriilor Fourier  ne spune ca orice
%semnal periodic poate fi aproximat printr-o suma infinita de sinusuri si
%cosinusuri de diferite frecvente, fiecare ponderat cu un anumit coeficient. 
